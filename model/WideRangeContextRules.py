...
class AmbitusRule(WideRangeContextRule):
    ...
    def check(self) -> float:
        ...
        minimum = min(midis)
        maximum = max(midis)
        return 0 if abs(maximum - minimum) >= 18 else 100


class LineShapeRule(WideRangeContextRule):
    max_one_direction = 10
...
class RhythmicRule(WideRangeContextRule):
    ...
    def check(self) -> float:
        ...
        return abs(self.rhythmic_percentage - (number_of_additional_triplets / (2 * number_of_quarters))) * 100
